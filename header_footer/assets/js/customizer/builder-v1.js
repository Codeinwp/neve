		...
		var Builder = {
			...
			drag_drop: function() {
				...
				$(".hf--device-panel", that.container).each(function() {
					...
					var sortable_ids = [];
					that.panels[device] = {};
					$(".hf--cb-items", panel).each(function(index) {
					...
					$(
						".hf-available-items .grid-stack-item",
						panel
					).draggable({
						revert: "invalid",
			...
			addNewWidget: function($item, row) {
				...
				var panel = that.container.find(
					".hf--device-panel.hf--panel-" +
						that.activePanel
				);
				var el = row;
				if (!_.isObject(el)) {
					el = panel.find(".hf--cb-items").first();
				}
			...
			addPanel: function(device) {
				...
				var template_id = "tmpl-hf--cb-panel";
			...
			addAvailableItems: function() {
				...
				_.each(that.devices, function(device_name, device) {
					var $itemWrapper = $(
						'<div class="hf-available-items" data-device="' +
							device +
							'"></div>'
					);
					...
					_.each(device_data, function(items, row_id) {
						if (!_.isUndefined(items)) {
							_.each(items, function(node, index) {
								var item = $(
									'.hf-available-items[data-device="' +
										device +
