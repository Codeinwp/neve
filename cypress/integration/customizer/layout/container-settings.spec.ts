...
describe('Container Settings', () => {
	...
	it('Setup customizer container width.', () => {
		...
		Object.keys(devices).map((device) => {
			cy.get('@control')
				.find('button.' + device)
				.click();
			cy.get('@control').find('input[type=number]').type('{selectall}').type(devices[device]);
		});
	...
	it('Setup customizer container style.', () => {
		...
		cy.get('#accordion-panel-neve_layout').click();
		cy.get('#accordion-section-neve_container').click();
		cy.get('#_customize-input-neve_default_container_style').select('full-width');
		cy.get('#_customize-input-neve_blog_archive_container_style').select('full-width');
		cy.get('#_customize-input-neve_single_post_container_style').select('full-width');
		cy.get('#save').click();
		cy.wait('@customizerSave').then((interception) => {
			expect(interception.response.body.success).to.be.true;
			expect(interception.response.statusCode).to.equal(200);
	...
	it('Go back to defaults.', () => {
		...
		cy.get('#accordion-section-neve_container').click();
		cy.get('#customize-control-neve_container_width').as('control');
		cy.get('#_customize-input-neve_default_container_style').select('contained');
		cy.get('#_customize-input-neve_blog_archive_container_style').select('contained');
		cy.get('#_customize-input-neve_single_post_container_style').select('contained');
		...
		devices.map((device) => {
			cy.get('@control')
				.find('button.' + device)
				.click();
			cy.get('@control').find('button').contains('Reset').should('be.visible').click();
		});
		cy.get('#save').click();
		cy.wait('@customizerSave').then((interception) => {
			expect(interception.response.body.success).to.be.true;
