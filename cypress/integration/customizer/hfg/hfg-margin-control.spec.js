describe('Header Builder Margin Control', () => {
	before(() => {
		...
		cy.get('#customize-control-primary-menu_tabs [data-tab="layout"]').should('be.visible').click();
		...
		cy.get('#customize-control-primary-menu_component_margin .top-input').should('have.value', '2');
		cy.get('#customize-control-primary-menu_component_margin .bottom-input').should(
			'have.value',
			'2',
		);
		cy.get('#customize-control-primary-menu_component_margin .left-input').should(
			'have.value',
			'2',
		);
		cy.get('#customize-control-primary-menu_component_margin .right-input').should(
			'have.value',
			'2',
		);

		// Click on Reset button
		cy.get('#customize-control-primary-menu_component_margin button.reset').click();

		// Check if Reset button works
		cy.get('#customize-control-primary-menu_component_margin .top-input').should('have.value', '0');
		cy.get('#customize-control-primary-menu_component_margin .bottom-input').should(
			'have.value',
			'0',
		);
		...
		// Unlink values
		cy.get('#customize-control-primary-menu_component_margin button.link').click();
