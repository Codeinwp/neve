describe('Header Row Background Control', () => {
	before('Setup Customizer Control', () => {
		...
		cy.get('.hfg--cb-body > .hfg--panel-desktop > .hfg--cp-rows > .hfg--row-main')
			.should('be.visible')
			.trigger('mouseover');
		...
		// Alias control.
		cy.get('#customize-control-hfg_header_layout_main_background .neve-background-control').as(
			'bgCtrl',
		);

		cy.get('@bgCtrl').should('be.visible');
		cy.get('@bgCtrl').children().should('have.length', 3);
		cy.get('@bgCtrl').find('.neve-background-type-control').children().should('have.length', 2);
