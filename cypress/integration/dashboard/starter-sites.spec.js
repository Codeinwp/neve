...
const BEFORE = () => {
	...
	cy.server()
	cy.route( 'https://api.themeisle.com/sites/*/wp-json/ti-demo-data/data*' )
		.as( 'loadRemoteData' )
...
describe( 'Starter Sites - Search', () => {
	...
	it( 'No Results', () => {
		...
		cy.get( '@search' ).type( '=' )
		cy.get( '@starterSitesTab' )
			.find( '.starter-site-card' )
			.should( 'not.exist' )
...
describe( 'Editor Switching | Categories Filtering | Infinite Scroll', () => {
	beforeEach( () => BEFORE() )
	...
	it( 'Editor Siwtcher', () => {
		...
		cy.get( '@starterSitesTab' ).find( '.editor-tabs' ).as( 'tabs' )
		cy.get( '@tabs' ).scrollIntoView()
		cy.get( '@tabs' ).find( 'a.gutenberg' ).should( 'have.class', 'active' )
		editors.map( editor => {
			cy.get( '@tabs' )
				.find( 'a' )
				.contains( editor )
				.closest( 'a' )
				.as( 'editorTab' )
			cy.get( '@editorTab' ).click()
			cy.get( '@editorTab' ).find( 'count' ).should( 'not.contain', 0 )
			cy.get( '@editorTab' ).should( 'have.class', 'active' )
		} )

		cy.get( '@tabs' ).contains( GUTENBERG ).click()

		cy.viewport( 'iphone-x' )
		cy.get( '@starterSitesTab' )
			.find( '.ob-dropdown.editor-selector button.ob-dropdown' )
			.as( 'editorSelect' )
	...
	it( 'Categories', () => {
		...
		cy.get( '@starterSitesTab' )
			.find( '.categories-selector' )
			.as( 'dropdown' )
		CATEGORIES.map( CAT => {
			cy.get( '@dropdown' ).find( 'button' ).click()
			cy.get( '@dropdown' ).find( 'ul.options' ).should( 'be.visible' )
			cy.get( '@dropdown' ).find( 'a' ).contains( CAT ).click()
		} )
		cy.get( '@dropdown' ).find( 'button' ).click()
		cy.get( '@dropdown' ).find( 'a' ).contains( ALL ).click()
	} )
} )
...
describe( 'Import UI', () => {
	...
	it( 'Does Not Import PRO Demo Site', () => {
		...
		cy.get( '@starterSitesTab' ).find( '.editor-tabs' ).as( 'tabs' )
		cy.get( '@tabs' ).find( 'a' ).contains( THRIVE ).click()
		cy.get( '@starterSitesTab' )
			.find( '.starter-site-card' )
...
describe( 'Import Process', () => {
	...
	beforeEach( () => BEFORE() )
	it( 'Imports properly', () => {
		cy.route( 'POST', HOMEURL + '/wp-json/ti-sites-lib/v1/install_plugins' )
			.as( 'plugins' )
		cy.route( 'POST', HOMEURL + '/wp-json/ti-sites-lib/v1/import_content' )
			.as( 'content' )
		cy.route( 'POST', HOMEURL + '/wp-json/ti-sites-lib/v1/import_theme_mods' )
			.as( 'mods' )
		cy.route( 'POST', HOMEURL + '/wp-json/ti-sites-lib/v1/import_widgets' )
			.as( 'widgets' )
