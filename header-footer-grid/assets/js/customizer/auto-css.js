...
( function( $, api ) {
    ...
    CustomifyAutoCSS.prototype.run = function( setting_name ){
        ...
        if ( $( '#hfg-google-font-css' ).length <= 0 ) {
            $( 'head' ).prepend( "<link rel='stylesheet' id='hfg-google-font-css'  href='' type='text/css' media='all' />" );
        }

        if ( url ) {
            //css_code = "\r\n@import url('"+url+"');\r\n\r\n"+css_code;
            $( '#hfg-google-font-css' ).attr( 'href', url );
        } else {
            $( '#hfg-google-font-css' ).remove();
        }

        css_code = css_code.trim();
        if ( $( '#hfg-style-inline-css' ).length <= 0 ) {
            $( 'head' ).append( "<style id='hfg-style-inline-css' type='text/css'></style>" )
        }
        $( '#hfg-style-inline-css' ).html( css_code );
